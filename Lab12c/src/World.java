...
public class World {
	 ...
	 * 
	 * @param filename - the String name of a file that will hold the configuration
	...
	// called as avatar coordinates and torch radius
	// should call recursive method
	{
		setSeen(false);
		int result = light(x, y, 0, 0, r);
	...
	private int light(int xCtr, int yCtr, int deltaX, int deltaY, double r) {
		// implement forest blocking

		// if(deltaX >= (int) r || deltaY >= (int) r || deltaX <= (int) -1.0*r || deltaY
		// <= (int) -1.0*r){
		// }

		if (Math.sqrt(Math.pow(deltaX, 2) + Math.pow(deltaY, 2)) >= r) {
			return 0;// case out of radius

		}

		if (xCtr + deltaX < 0 || yCtr + deltaY < 0 || xCtr + deltaX >= this.tiles.length
				|| yCtr + deltaY >= this.tiles[xCtr + deltaX].length) {
			return 0;// case out of bounds
		}

		if (this.tiles[xCtr + deltaX][yCtr + deltaY].getSeen()) {
			return 0;
		}

		try {
			System.out.println("x: " + (xCtr + deltaX) + " y: " + (yCtr + deltaY));
			this.tiles[xCtr + deltaX][yCtr + deltaY].setLit(true);
			this.tiles[xCtr + deltaX][yCtr + deltaY].setSeen(true);
		} catch (Exception e) {
		}

		int a = light(xCtr, yCtr, deltaX + 1, deltaY, r); // lr1
		int b = light(xCtr, yCtr, deltaX, deltaY + 1, r); // up
		int c = light(xCtr, yCtr, deltaX - 1, deltaY, r); // lr2
		int d = light(xCtr, yCtr, deltaX, deltaY - 1, r); // down
	...
	/**
	 * Set all the seen values used in the DFSearch
	 * 
